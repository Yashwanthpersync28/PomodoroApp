  ...
  return (
    ...
    <View style={[{ width: 5,height:52.5,backgroundColor:taskColor },radius(0,0,0,8,8)]}></View>
      ...
      <Icon name={"circle"} type={Icons.Entypo} style={[styles.tomotoRed, fontSize(25), marginPosition(0, 20)]} /> }
              <Text style={[selectedTask ===taskSelected ?styles.lightGray:( darkMode?styles.lightWhite:styles.black),fontSize(18)]}>{selectedTask}</Text></View>
          <TouchableOpacity onPress={selectedTask === taskSelected ? ()=>setCurrentModal(1):()=>clearTask()}><Icon name={selectedTask ==='Select Task'? "chevron-down":'x'} type={Icons.Feather} style={[darkMode?styles.lightWhite:styles.black,fontSize(25)]} /></TouchableOpacity>
            </View>
          </TouchableWithoutFeedback>
        </View>
  )
