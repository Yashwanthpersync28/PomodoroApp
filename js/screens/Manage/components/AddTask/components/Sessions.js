  ...
  return (
    ...
    <View style={[styles.row,styles.allCenter]}>
      {data.map((num,index)=>{
       ...
       <View key={index} style={[{height:28,width:28},radius(50),session===index+1 ? styles.bgOrange:styles.bgWhite,styles.allCenter,marginPosition(0,10)]}>
             <Text style={[session===index+1 ? styles.white:styles.black]}>{num}</Text>
       </View>
       </TouchableOpacity>
        )
      })}
