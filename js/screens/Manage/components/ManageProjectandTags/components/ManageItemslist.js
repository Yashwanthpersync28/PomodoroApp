...
export const ManageItemslist = ({Darkmode , data, showProjects , optionOne , optionTwo , handleArchieveProjects , handleArchieveTags, handleoptionOneProject,handleoptionOneTags,showArchievedlists}) => {
  console.log('data', data);
  const [selectedItem, setSelectedItem] = useState(null);
      ...
      {data.map((item,index) => (
          ...
          <View key={index} style={[zIndex(0)]}> 
            <View style={[styles.row,Darkmode?styles.bgdarkmodeBlack:styles.bgMilkyWhite, { height: heightValue(14) }, borderColor(Darkmode?Colors.darkmodeBorderColor:'#f2f0f0'), borderWidth(0,1), { position: 'relative' },zIndex(0)]}>
              ...
              <View style={[flex(1), styles.centerVertical]}>
                <Text style={[fontSize(18),Darkmode?styles.inputColor: styles.black]}>{item.name}</Text>
              </View>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => handleItemPress(item)} style={[flex(0.2), styles.flexEnd]}>
              <View style={[flex(1), styles.allCenter]}>
                <Icon name={'dots-three-vertical'} type={Icons.Entypo} style={[fontSize(20),Darkmode?styles.inputColor:styles.black]} />
              </View>
              </TouchableOpacity>
            </View>
            {/* options */}
