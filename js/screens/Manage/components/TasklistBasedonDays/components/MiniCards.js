...
export const MiniCards = ({number,name}) => {
  return (
    <View style={[styles.column,styles.allCenter,{height:heightValue(10),width:widthValue(2.4)},styles.bgWhite,radius(5),marginPosition(0,0,13)]}>
         <Text style={[fontSize(22),styles.Orange,fontWeight('bold')]}>{number}</Text> 
         <Text style={[fontSize(14),styles.gray,marginPosition(10)]}>{name}</Text>    
    </View>
  )
}
